...
RSpec.describe "real world edgecases", :realworld => true, :sometimes => true do
  ...
  it "doesn't hang on big gemfile" do
    ...
    if Bundler.feature_flag.bundler_3_mode?
      expect(out).to include("BUNDLER: Finished resolution (989 steps)")
    else
  ...
  it "doesn't hang on tricky gemfile" do
    ...
    if Bundler.feature_flag.bundler_3_mode?
      expect(out).to include("BUNDLER: Finished resolution (869 steps)")
    else
  ...
  it "doesn't hang on nix gemfile" do
    ...
    if Bundler.feature_flag.bundler_3_mode?
      expect(out).to include("BUNDLER: Finished resolution (1826 steps)")
    else
